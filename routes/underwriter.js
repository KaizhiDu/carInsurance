...
router.post("/getQuoteById", function (req, res) {
    const id = mongoose.Types.ObjectId(req.body.id);

    Quote.findOne({_id: id}, function (err, quote) {
        if(err){
            console.log(err);
        }else{
            //console.log(quote);
...
router.post("/saveQuote", function (req, res) {
    const id = mongoose.Types.ObjectId(req.body.id);
    const appriveStatus = req.body.appriveStatus;
    const comment = req.body.comment;
console.log(comment);

    Quote.updateOne({_id: id}, {$set : { comment : comment, uwprove : appriveStatus }}, function (err) {
        if(err){
            console.log(err);
        }else{
            console.log("successful update");
